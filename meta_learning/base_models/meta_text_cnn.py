from abstract_meta_base_model import AbstractMetaBaseModel
import torch
...
class DenseClassifier(AbstractMetaBaseModel):
    def __init__(self, sentence_length, embedding_dim, n_filters, filter_size, pool_size, hidden_size, num_classes, init_classifier=True, init_seed=0, randomness_seed=0):
        ...
        states = self.set_rng_state(self.init_seed)
        ...
        self.randomness_states = self.get_rng_state()
        self.restore_rng_state(states)

    def forward(self, embedded_text, params=None):
        states = self.get_rng_state()
        self.restore_rng_state(self.randomness_states)
...
class MetaSimpleCnnTextProto(AbstractMetaBaseModel):
    def __init__(self, sentence_length, embedding_dim, n_filters, filter_size, pool_size, hidden_size, num_classes, init_classifier=False, init_seed=0, randomness_seed=0):
...
class MetaTextCnn(AbstractMetaBaseModel):
    def __init__(self, sentence_length, embedding_dim, n_filters, filter_sizes, pool_size, hidden_size, num_classes, init_seed=0, randomness_seed=0):
