...
for DATASET_PATH in ['sst2', 'ag_news', 'snips']:
    ...
    for MODEL in ['flan-t5', 'llama2', 'mistral', 'zephyr']:
        ...
        for shot in [2, 4, 6, 8, 10]:
            ...
            if shot == 2:
                ...
                base_path = os.path.join('results', 'stability', f'{L3D}_{MODEL}_base', f'shot_{shot}', DATASET_PATH)
            ...
            print(f'{np.mean(golden_model):.2f}')
            print(f'{np.std(golden_model):.2f}')
            ...
            for factor in ['sample_order', 'sample_choice']:
                all = failed = 0
                results[factor]['full'] = []
                results[factor]['agg'] = {}
